...
{#if offline && !dismissed}<aside class="offline-banner" role="status"><WifiOff /><span><b>You are offline.</b> Check-ins continue with the labeled deterministic-v2 policy. Live adaptation, Model Room jobs, and the optional lofi stream need a connection.</span><button aria-label="Dismiss offline notice" onclick={() => dismissed = true}><X /></button></aside>{/if}
